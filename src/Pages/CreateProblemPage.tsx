...
export const ProblemDataSchema = z.object({
  ...
  referenceSolutions: z.string().optional(),
  companyName: z.array(z.string()).optional(),
});

type ProblemData = z.infer<typeof ProblemDataSchema>;

const problem = async (proData: ProblemData) => {
  const { data } = await createProblem(proData);
  return data;
};
...
export default function CreateProblemPage() {
  const {
    ...
    resolver: zodResolver(ProblemDataSchema),
    defaultValues: {
      description: "",
      difficulty: "EASY",
      topic: [],
  ...
  const handleToggleTopic = (selectedTopic: string) => {
    if (topics?.includes(selectedTopic)) {
      setValue(
        "topic",
        topics.filter((t) => t !== selectedTopic),
        { shouldValidate: true, shouldDirty: true }
      );
    } else {
      setValue("topic", [...topics!, selectedTopic], {
        shouldValidate: true,
        shouldDirty: true,
      });
    }
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: ["problemData"],
    mutationFn: problem,
  });

  const onSubmit = (data: ProblemData) => {
    mutate(data);
                          ...
                          {TOPICS.map((topic) => (
                            ...
                            >
                              <input
                                ...
                                className="border-gray-600 checked:bg-amber-500 checked:border-amber-500"
                              />
                              <label
                                htmlFor={`topic-${topic}`}
